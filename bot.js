...
bot.command('prices', async (ctx) => {
    try {
        ...
        ctx.reply(`❌ Error fetching prices: ${error.message}`);
    }
});

// Command to get gas fees for Solana
bot.command('gas', async (ctx) => {
    try {
        const feeCalculator = await connection.getFeeCalculatorForBlockhash(await connection.getRecentBlockhash());
        const fee = feeCalculator.value.lamportsPerSignature;
        ctx.reply(`⛽ **Solana Gas Fee**: ${fee / 1000000000} SOL per transaction.`);
    } catch (error) {
